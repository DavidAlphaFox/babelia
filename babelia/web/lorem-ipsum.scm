(define-module (babelia web lorem-ipsum))


(define (newline? char)
  (char=? char #\newline))

(define (replace-newline-with-space char)
  (if (newline? char)
      #\space
      char))

(define (replace-newlines-with-spaces string)
  (string-map replace-newline-with-space string))

(define (string->tokens string)
  (string-split (replace-newlines-with-spaces string) #\space))

(define %lorem-ipsum (string->tokens "Sed ut perspiciatis, unde omnis
...
(define-public (lorem-ipsum length)
  (string-join (map (lambda _ (choice %lorem-ipsum)) (iota length)) " "))
